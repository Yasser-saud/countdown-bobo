		...
		<main className="relative flex min-h-screen w-full items-center justify-center overflow-hidden xl:h-screen">
			...
			<div className="relative z-[100] mx-auto w-full px-4 py-16 text-center">
				<motion.h1
					...
					className={cn(
						'from-foreground via-foreground/80 to-foreground/40 mb-6 cursor-crosshair bg-linear-to-b bg-clip-text text-3xl font-semibold text-transparent sm:text-7xl',
					)}
				>
					<span className="bg-primary from-foreground to-primary via-muted-foreground bg-clip-text text-transparent dark:bg-linear-to-b">
						بشاير
					</span>
				</motion.h1>
				<motion.div
					...
					className="w-full  grid grid-cols-4 gap-1 sm:grid-cols-4"
				>
					{renderCountdown.map((item, i) => (
						<motion.div
							...
							animate={{scale: 1, x: 0}}
							transition={{duration: 0.2, delay: 1 + i * 0.2, type: 'spring'}}
							className={cn(
